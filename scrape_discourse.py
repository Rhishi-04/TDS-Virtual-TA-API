...
def clean_post(post):
    return {
        **post,
        "author": None,  # Remove PII
        "mentioned_users": []  # Remove PII
    }
        ...
        if DATE_FROM <= created_at <= DATE_TO:
            ...
            for post in posts:
                post_data = {
                    ...
                    "created_at": parse_date(post["created_at"]).isoformat(),
                    "updated_at": parse_date(post["updated_at"]).isoformat() if post.get("updated_at") else None,
                ...
                filtered_posts.append(clean_post(post_data))
            time.sleep(uniform(1.5, 3.0))
